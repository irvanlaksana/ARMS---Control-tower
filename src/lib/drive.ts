...
export async function createDriveFolder(name: string, parentId?: string | null): Promise<DriveFolderResult> {
  let resp: Response;
  try {
    resp = await fetch('/api/drive/create-folder', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, parentId: parentId || undefined }),
    });
  } catch (netErr: any) {
    throw new Error(`Tidak dapat menghubungi server API: ${netErr?.message || netErr}`);
  }
  ...
  try {
    ...
    if (rawText.includes('A server error') || resp.status === 500) {
      ...
      cleanMsg = 'Endpoint /api/drive/create-folder tidak ditemukan (404).';
    }
...
export async function ensureDrivePath(segments: string[], rootId?: string | null): Promise<DrivePathResult> {
  let resp: Response;
  try {
    resp = await fetch('/api/drive/ensure-path', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  try {
    ...
    if (rawText.includes('A server error') || resp.status === 500) {
      ...
      cleanMsg = 'Endpoint /api/drive/ensure-path tidak ditemukan (404).';
    }
    throw new Error(cleanMsg);
  }
