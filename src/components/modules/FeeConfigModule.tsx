...
import { Settings, Plus, CheckCircle, ShieldCheck, Edit2, Trash2, X, PlusCircle, Search } from 'lucide-react';
...
export const FeeConfigModule: React.FC<FeeConfigModuleProps> = ({
  ...
  const filteredFees = useMemo(() => {
    return store.fees.filter((f) => {
      if (searchTerm) {
        const q = searchTerm.toLowerCase();
        const match = `${f.clientName} ${f.serviceName} ${f.feeType}`.toLowerCase();
        if (!match.includes(q)) return false;
      }
      return true;
    });
  }, [store.fees, searchTerm]);
