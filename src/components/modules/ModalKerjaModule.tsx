...
export const ModalKerjaModule: React.FC<ModalKerjaModuleProps> = ({
  ...
  const filteredList = useMemo(() => {
    return workingCapitalList.filter((item) => {
      const matchesSearch = 
  ...
  }, [workingCapitalList, searchTerm, typeFilter]);

  const modalPagination = usePagination<WorkingCapitalTransaction>(filteredList, 10);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800">
              {modalPagination.pageItems.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-500 text-xs">
                    {workingCapitalList.length === 0
                      ? 'Belum ada data transaksi modal kerja. Klik tombol + Catat Transaksi Modal Kerja untuk menambahkan setoran atau alokasi modal.'
                      : 'Tidak ada transaksi modal kerja yang sesuai dengan filter atau pencarian.'}
                  </td>
                </tr>
              ) : (
        ...
        <Pagination
          ...
          totalItems={modalPagination.totalItems}
          pageSize={modalPagination.pageSize}
          onPageChange={modalPagination.setPage}
          onPageSizeChange={modalPagination.setPageSize}
        />
      </div>
