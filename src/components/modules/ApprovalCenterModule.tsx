...
import { CheckSquare, CheckCircle, XCircle, Clock, Plus, Edit2, Trash2, X, Filter, Search } from 'lucide-react';
...
export const ApprovalCenterModule: React.FC<ApprovalCenterModuleProps> = ({
  ...
  const filteredApprovals = useMemo(() => {
    return store.approvals.filter((app) => {
      if (filterModule !== 'ALL' && app.module !== filterModule) return false;
      if (filterStatus !== 'ALL' && app.status !== filterStatus) return false;
      if (searchTerm) {
        const q = searchTerm.toLowerCase();
        const match = `${app.requestNo} ${app.module} ${app.title} ${app.description || ''} ${app.requestedBy} ${app.targetReference || ''}`.toLowerCase();
        if (!match.includes(q)) return false;
      }
      return true;
    });
  }, [store.approvals, filterModule, filterStatus, searchTerm]);
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Cari permohonan / judul..."
              className="bg-slate-950 border border-slate-800 rounded-lg pl-8 pr-3 py-2 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-500 w-full sm:w-56"
            />
          </div>
