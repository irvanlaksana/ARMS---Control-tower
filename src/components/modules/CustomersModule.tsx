...
import { Users, Plus, Edit2, Trash2, AlertTriangle, Upload, Image as ImageIcon, Eye, X, CheckCircle, UserCheck, Clock, AlertCircle, Download, FileSpreadsheet, UploadCloud, ClipboardList, CheckCircle2, Loader2, ExternalLink, Search } from 'lucide-react';
...
export const CustomersModule: React.FC<CustomersModuleProps> = ({ store, currentUser, onUpdateStore }) => {
  ...
  const [statusFilter, setStatusFilter] = useState<'ALL' | DebtorProcessStatus>('ALL');
  ...
  const filteredCustomers = useMemo(() => {
    return (store.customers || []).filter((c) => {
      ...
      const matchesSearch =
        ...
        (c.vehiclePoliceNo && c.vehiclePoliceNo.toLowerCase().includes(term)) ||
        (c.address && c.address.toLowerCase().includes(term));

      if (!matchesSearch) return false;

      if (statusFilter !== 'ALL') {
        const proc = getDebtorProcessStatus(c.id);
        if (proc.status !== statusFilter) return false;
      }
      return true;
    });
  }, [store.customers, searchTerm, statusFilter, store.cases, store.assets, store.sks, store.assignments]);

  const customerPagination = usePagination<Customer>(filteredCustomers, 10);
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>, target: 'KTP' | 'STNK') => {
    const files = Array.from(event.target.files ?? []) as File[];
    if (files.length === 0) return;
    setIsUploadingPhoto(true);

    const folderId = store.settings?.googleDriveFolderId || undefined;
    for (const file of files) {
      const customPrefix = `${target}_${(fullName || 'DEBITUR').replace(/[^a-zA-Z0-9]/g, '_')}`;
      const result = await uploadFileToDrive(file, folderId, customPrefix);
      const photoUrl = result.webViewLink || '';
      if (photoUrl) {
        if (target === 'KTP') {
          setKtpPhotoUrl(photoUrl);
        } else {
          setStnkPhotoUrls((prev) => [...prev, photoUrl]);
        }
      }
    }
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 bg-slate-900 border border-slate-800 p-1 rounded-lg overflow-x-auto">
          {(['ALL', 'NEW', 'PROCESS', 'ASSIGNED', 'CLOSED'] as const).map((st) => (
            ...
            >
              {st === 'ALL' && 'Semua Debitur'}
              {st === 'NEW' && 'Baru'}
              {st === 'PROCESS' && 'Sedang Proses'}
              {st === 'ASSIGNED' && 'Tugas Lapangan'}
              {st === 'CLOSED' && 'Closed / Lunas'}
            </button>
          ))}
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800">
              {customerPagination.pageItems.length === 0 ? (
                <tr>
                  <td colSpan={canEdit ? 9 : 8} className="py-12 text-center text-slate-500 text-xs">
                    {store.customers.length === 0
                      ? 'Belum ada data debitur. Silakan tambah debitur baru atau import file.'
                      : 'Tidak ada data debitur yang cocok dengan pencarian atau filter.'}
                  </td>
                </tr>
              ) : (
                    ...
                    <div className="relative group">
                      ...
                      {ktpPhotoUrl.startsWith('http') && (
                        <a
                          ...
                          className="absolute bottom-1 left-1 bg-slate-950/85 text-indigo-300 hover:text-indigo-200 text-[10px] px-1.5 py-0.5 rounded flex items-center gap-1 border border-indigo-500/30 font-medium"
                        >
                ...
                <div className="space-y-1.5">
                  <label className="flex items-center justify-center w-full gap-2 border border-dashed border-slate-700 rounded-lg p-2 text-xs text-slate-300 bg-slate-950 cursor-pointer hover:border-indigo-500 transition">
                    ...
                    <input
                      ...
                      className="hidden"
                      onChange={(e) => handleImageUpload(e, 'STNK')}
                    />
                  </label>
