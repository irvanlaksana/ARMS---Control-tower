...
${firm || 'Kantor Advokat & Konsultan Hukum Mitra'}`;
  ...
  }, [noticesList, store.cases, clientFilter, searchTerm]);

  const lawyerPagination = usePagination<LawyerNotice>(filteredNotices, 10);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
          ...
          <table className="w-full text-left text-xs text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800">
              {lawyerPagination.pageItems.length === 0 ? (
                ...
                lawyerPagination.pageItems.map((n) => {
                  const parentCase = store.cases.find((c) => c.id === n.caseId || c.caseNo === n.caseNo);
                  const isPer = parentCase?.clientType === 'PERORANGAN';
        ...
        <Pagination
          page={lawyerPagination.page}
          totalPages={lawyerPagination.totalPages}
          totalItems={lawyerPagination.totalItems}
          pageSize={lawyerPagination.pageSize}
          onPageChange={lawyerPagination.setPage}
          onPageSizeChange={lawyerPagination.setPageSize}
        />
      </div>
